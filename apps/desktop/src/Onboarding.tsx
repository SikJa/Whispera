...
export default function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  return <main className="setup-shell" lang={lang}>
    ...
    <section key={step} className="setup-step">
      {step === 0 && <><ShieldCheck size={28} /><h1>{t('Tu voz, tu configuración', 'Your voice, your setup')}</h1><p>{t('Whispera envía el audio y el vocabulario de tu diccionario a Groq para transcribir. Usás tu propia cuenta; sus límites y condiciones aplican. Por defecto, los audios largos se envían por tramos mientras hablás. Cancelar no retira lo ya enviado; podés desactivar el envío anticipado en Configuración.', 'Whispera sends audio and your dictionary vocabulary to Groq for transcription. You use your own account; its limits and terms apply. Long recordings upload in parts while you speak by default. Cancelling cannot withdraw submitted audio; you can disable background transcription in Settings.')}</p><p>{t('Las grabaciones y el historial se guardan en esta PC para recuperación. No se borran automáticamente. La clave se guarda en el almacén de credenciales de Windows, no en el repositorio.', 'Recordings and history stay on this PC for recovery. They are not automatically deleted. Your key is stored in Windows Credential Manager, not in the repository.')}</p><p>{t('Sin cuenta de Whispera ni modelos locales. Podés salir desde el icono de bandeja.', 'No Whispera account or local models. Quit from the tray icon.')}</p></>}
