    ...
    <div className="desktop-main">
      <section className="desktop-content">
        ...
        {route === "transcription" && <>
          ...
          <button disabled={busy || !api.native} onClick={() => run(api.importKey, "Clave anterior importada en Windows")}><Download size={16} />Usar clave de Whispera anterior</button>
        ...
        {route === "diagnostics" && <><div className="diagnostic-line"><span>Interfaz</span><strong>React + Motion</strong><Check size={16} /></div><div className="diagnostic-line"><span>Motor nativo</span><strong>{api.native ? "Tauri / Rust" : "No conectado"}</strong></div><div className="diagnostic-line"><span>Proveedor</span><strong>Groq</strong></div><h3>Datos anteriores</h3><p className="muted-note">Importación manual desde la carpeta .local. Los archivos originales se conservan.</p><button disabled={busy || !api.native} onClick={() => run(api.importLegacy, "Datos importados") }><Download size={16} />Importar configuración e historial</button><h3>Eventos</h3><pre className="log-view">{data.logs.join("\n") || "Sin eventos en esta sesión."}</pre></>}
        </SectionReveal>
      </section>
    </div>
